...
// ─── Tool registration moved to ToolRegistration.cpp ───────────────────────
...
{
    // During sketch editing, undo the last sketch action (not the whole sketch)
    if (m_sketchEditor && m_sketchEditor->isEditing()) {
        if (m_sketchEditor->undoLastAction()) {
            m_viewport->update();
            m_properties->refreshSketchStats();
            statusBar()->showMessage(tr("Sketch: undid last action"));
        } else {
            statusBar()->showMessage(tr("Sketch: nothing to undo"));
        }
        return;
    }
...
void MainWindow::showConfirmBar(const QString& toolName)
{
    ...
    m_confirmBar->raise();
    // Confirm bar kept hidden — status bar already shows Enter/Escape hints.
}
