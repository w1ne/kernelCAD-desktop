...
RibFeature::RibFeature(std::string id, RibParams params)
    : m_id(std::move(id)), m_params(std::move(params))
{}

TopoDS_Shape RibFeature::execute(kernel::OCCTKernel& kernel,
                                  ...
                                  const sketch::Sketch* sketch) const
{
    ...
    return kernel.rib(body, profileShape, m_params.thickness, m_params.depth);
}
